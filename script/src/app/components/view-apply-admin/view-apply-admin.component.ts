...
@Component({
  selector: 'app-view-apply-admin',
  templateUrl: './view-apply-admin.component.html',
  styleUrl: './view-apply-admin.component.scss',
})
export class ViewApplyAdminComponent {
  isVisibleAdd = false;
  ...
  ngOnInit(): void {
    this.jobApply.viewJobApplicationByAdmin(1).subscribe(
      (res) => {
  ...
  pageActive(pageActive: number): void {
    this.jobApply.viewJobApplicationByAdmin(pageActive).subscribe(
      (res) => {
        this.listJob = res.listResults;
        this.totalPage = res.totalPage;
        this.page = res.page;
